...
exports.getOpenSubtitlesLanguage = function(key){
    for(var i=0; i<languages.supported.length;i++){
        if(key.toUpperCase() == languages.supported[i]['Id'].toUpperCase()
          || (languages.supported[i]['SubLanguage'] 
            && key.toUpperCase() == languages.supported[i]['SubLanguage'].toUpperCase())){
            
            return languages.supported[i]['Id'];
        }
    }
};
